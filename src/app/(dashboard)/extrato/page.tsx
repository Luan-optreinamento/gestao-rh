...
export default function ExtratoPage() {
  ...
  const conciliadas = txs.filter(t => t.conciliada && t.descricao !== '(ignorada)')
    ...
    <div>
      ...
      <div className="flex gap-1 bg-gray-100 p-1 rounded-lg w-fit mb-5">
        {(['upload', 'historico'] as const).map(a => (
          <button key={a} onClick={() => setAba(a)} className={`px-4 py-1.5 rounded-md text-sm font-medium transition-colors ${aba === a ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>
      ...
      {aba === 'historico' && (
        <Card title="Transações Conciliadas">
          {!conciliadas.length
            ? <p className="text-sm text-gray-400 text-center py-6">Nenhuma transação conciliada ainda.</p>
                  ...
                  {conciliadas.map(t => {
                    ...
                    return (
                      <tr key={t.id} className="border-b border-gray-100 hover:bg-gray-50">
                        ...
                        <td className={`px-3 py-2.5 font-semibold ${parseFloat(t.valor) >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                          {fmt(parseFloat(t.valor))}
                        </td>
